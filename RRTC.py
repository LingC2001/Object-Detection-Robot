...
def find_obs_wrt_goal(goal_next, goal_list, obstacle_list, obs_radius, pre = None, stuck_start=None): # this is to ensure the goal is not in the obstacle list
    checking = True
    all_obstacles = []
    all_obs_coord = []
...
def plot_path(est,obs, name, pose=None, count = None, apple_list = None, lemon_list = None, orange_list = None, pear_list = None, strawberry_list = None):
    ...
    def planning(self):
        ...
        #TODO: Complete the planning method ----------------------------------------------------------------
            ...
            if d < self.expand_dis:
                nearby_node = self.steer(closest_node, new_node, self.expand_dis)
            ...
            rdm_node_end = self.get_random_node()
            end_exp_ind = self.get_nearest_node_index(self.end_node_list, rdm_node_end) 
            end_exp_node = self.end_node_list[end_exp_ind]
            dist_end,_ = self.calc_distance_and_angle(end_exp_node, rdm_node_end)

            new_end_node = self.steer(end_exp_node, rdm_node_end,self.expand_dis)
