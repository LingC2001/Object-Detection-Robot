...
class EKF:
    ...
    def __init__(self, robot):
        ...
        self.taglist = []
        self.robot.state = np.zeros((3, 1)) # nyan kyaw edit

        # Covariance matrix
        self.P = np.zeros((3,3))
        self.init_lm_cov = 1e-8
    ...
    def update(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # print("tag " + str(measurements[i].tag) + ": [ "+str(x1) + ", " + str(y1) +" ]")
            dist = np.sqrt((x-x1)**2 + (y-y1)**2)
            scaling = (dist + 1)*0.5

            R[2*i:2*i+2,2*i:2*i+2] = 1e-6
        ...
        dist = np.sqrt((x_update[0]-x[0])**2+(x_update[1]-x[1])**2)
        ...
        if True: # dist < 0.3:
            ...
            self.P = (np.eye(x_update.shape[0]) - K @ H) @ self.P
