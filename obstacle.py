...
class Rectangle(Polygon):
    ...
    def __init__(self, origin=np.zeros(2), width=100, height=20, position=None):
        self.width = width
        self.height = height # half height not height
        self.origin = origin
        self.pos = position

        v1 = origin + np.array([-width, height]) # top left
        v2 = origin + np.array([width, height]) # top right
        v4 = origin + np.array([-width, -height]) # bottom left
        v3 = origin + np.array([width, -height]) # bottom right
    ...
    def is_in_collision_with_points(self, points):
        ...
        #     dist.append(dx * dx + dy * dy)
            ...
            if self.pos == 'u' and point[1] > 1.5-self.height:
                return True
            elif self.pose == 'd' and point[1] < -1.5+self.height:
                return True
            elif self.pose == 'r' and point[0] > 1.5-self.width:
                return True
            elif self.pose =='l' and point[0] < -1.5+self.width:
                return True
